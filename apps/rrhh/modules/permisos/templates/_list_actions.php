<li class="sf_admin_action_solicitar_permisos">
    <a href="#" onclick="open_window_right(); solicitar_permisos(); return false;">Solicitar Permisos</a>
</li>
...
<?php 
  ...
  if($ver_global == FALSE){
    foreach ($rrhh_delegados as $rrhh_delegado) {
        ...
        if(isset($parametros['ver_permisos_global'])) $ver_global = TRUE;
    }
  }
  ...
  if($ver_unidad == FALSE) {
      foreach ($rrhh_delegados as $rrhh_delegado) {
        $parametros = sfYaml::load($rrhh_delegado->getParametros());
        if(isset($parametros['ver_permisos_unidad'])) $ver_unidad = TRUE;
    }
  }

  if($ver_unidad == TRUE) { ?>
      <li class="sf_admin_action_reporte_unidad_permisos">
        <?php echo link_to(__('Reporte de la Unidad', array(), 'messages'), 'permisos/reporteGlobal?tipo=unidad', array()) ?>
      </li>
  <?php }
  
  if($ver_global == TRUE) {
?>
<li class="sf_admin_action_reporte_global_permisos">
  <?php echo link_to(__('Reporte de Global', array(), 'messages'), 'permisos/reporteGlobal?tipo=global', array()) ?>
</li>
<?php } ?>