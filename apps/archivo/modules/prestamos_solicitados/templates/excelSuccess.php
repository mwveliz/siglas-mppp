...
if($excel)
{
        ...
        $objSheet->getStyle('K1')->applyFromArray($style_header);
        ...
        $objSheet->getColumnDimensionByColumn('10')->setWidth(25);
        $objSheet->getColumnDimensionByColumn('11')->setWidth(25);
        ...
        $objSheet->getStyle('K1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
        ...
        $objSheet->getCell('J1')->setValue('Fecha de Prestamo');
        $objSheet->getCell('K1')->setValue('Fecha de Expiracion');
...
//    $conmutador= 1;
    foreach ($excel as $datos)
    {
        ...
        //Decimo campo fecha de prestamo
        $archivo_prestamo_archivo = Doctrine::getTable('Archivo_PrestamoArchivo')->findByExpedienteIdAndFuncionarioId($datos->getId(), $sf_user->getAttribute('funcionario_id'));
        $field10= date('d-m-Y h:i:s A', strtotime($archivo_prestamo_archivo[0]->getCreatedAt()));
        
        //Decimo primer campo fecha de expiracion
        $field11= date('d-m-Y', strtotime($archivo_prestamo_archivo[0]->getFExpiracion()));
        ...
        $objSheet->getCell('J'.$row_index)->setValue($field10);
        $objSheet->getCell('K'.$row_index)->setValue($field11);
...
//        }
        ...
        $objSheet->getCell('J'.$row_index)->setValue();
        $objSheet->getCell('K'.$row_index)->setValue();
...
//        $conmutador+= 2;
    ...
    $nombre= 'expedientes_prestados_al_'.date('d-m-Y');
