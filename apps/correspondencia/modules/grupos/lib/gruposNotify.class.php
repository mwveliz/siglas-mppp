    ...
    if(!$boss) {
        ...
        foreach($unidad_autoridad as $value) {
                ...
                if(count($registro_funcionario_unidad_id) > 1) {
                    $html= '
                            <table width="100%">
                                ...
                                <tr>
                                    <td>El funcionario <b>' . $datos_funcio_afectado[0]['primer_nombre'].' '.$datos_funcio_afectado[0]['primer_apellido'] . '</b> ha sido agregado a otro grupo de <b>CORRESPONDENCIA</b></td>
                                </tr>
                                <tr>
                                    <td class="f14n">
                                        <div id="sf_admin_container" style="width: 230px; float: left">
                                            <div class="partial_find_eliminar partial" style="height: 20px;">
                                                <a href="javascript: group_correspondencia_eliminar(\''. $registro_funcionario_unidad_id->getId() .'\', \'$#$\', \'eventos\')" onclick="return confirm(\'Se eliminaran permisos de este funcionario sobre su Grupo. Esta usted seguro?\')">Eliminar de mi Grupo</a>
                                            </div>
                                        ...
                                        <div style="float: left; max-width: 490px; border: 1px dashed #bc6f00; padding: 3px">
                                            <font style="font-size: 10px; color: #683e02">Funcionario agregado al grupo <b>' . $datos_funcio_autor[0]['unombre'] . '</b> por <b>' . $datos_funcio_autor[0]['primer_nombre'].' '.$datos_funcio_autor[0]['primer_apellido'].'('.$datos_funcio_autor[0]['ctnombre'].')' . '</b>. Puede editar los privilegios de este Usuario desde la opci&oacute;n <a href="' . sfConfig::get('sf_app_correspondencia_url') . 'grupos">Permisos del Grupo de Correspondencia</a></font>
                                        </div>
                                    </td>
                                </tr>
                        </table>';
            ...
            if($html) {
                ...
                $comunicaciones_notificacion->setMetodoId(1); //cambio de grupo correspondencia
