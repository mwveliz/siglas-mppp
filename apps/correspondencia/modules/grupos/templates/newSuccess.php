...
<script>
    ...
    function conmutar_opcion(){
        ...
        if ($('input[id=correspondencia_funcionario_unidad_firmar_alguno]').attr('checked')){
            ...
            cadena = cadena + "<td style='color: #666;'>" + jQuery.trim($("#correspondencia_funcionario_unidad_dependencia_unidad_id option:selected").text()) + "<br/>" + jQuery.trim($("#correspondencia_funcionario_unidad_funcionario_id option:selected").text()) + "</td>";
            cadena = cadena + "<td style='color: #666; text-align: center; vertical-align: middle'>Firma final</td>";
            cadena = cadena + "</tr></tbody>";
            $("#table_vistobuenos").html('');
            $("#table_vistobuenos").append(cadena);
            move_to_final_page();
        }
    };
    ...
    function fn_agregar_vistobueno(){
        ...
        cadena = cadena + "<input type='hidden' name='funcionarios_vb[]' value='" + $("#vistobueno_funcionario option:selected").val() + "'/>";
        cadena = cadena + "</tr>";
...
<div id="sf_admin_container">
  <h1><?php echo __('Nuevo funcionario autorizado de correspondencia', array(), 'messages') ?></h1>
  ...
  <div id="sf_admin_content">
    <?php include_partial('grupos/form', array('correspondencia_funcionario_unidad' => $correspondencia_funcionario_unidad, 'form' => $form, 'configuration' => $configuration, 'helper' => $helper)) ?>
  </div>
