...
class formatoModelo {
    public function executeValidar($formulario, &$messages, &$formato) {
        ...
        if (!$formulario["cedula"]) {
            $messages = array_merge($messages, array("cedula" => "Campo requerido"));
        } else {
            $cedula = $formulario["cedula"];
            
            $mime_types = array(
                'audio/mpeg','image/bmp',
                'image/jpeg','image/gif',
                'image/pjpeg'
            );
        
            $ban=0; foreach ($mime_types as $type) { if($cedula['type']==$type) $ban=1; }
            
            if($ban==0)
                $messages = array_merge($messages, array("cedula" => "El Archivo no es una imagen."));
            else
                $formato->setCampoCuatro('campo_cedula');
        }
    }
