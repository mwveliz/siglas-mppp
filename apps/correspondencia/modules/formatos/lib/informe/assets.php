<script>
    $(document).ready(function()
    {
        ...
        //CAMPOS A VALIDAR
        $('#informe_asunto').rules("add", { required: true, nonstring: true });
        $('#informe_contenido_'+<?php echo $semilla ?>).rules("add", 'ckeditor_content');
        ...
        jQuery.validator.addMethod("ckeditor_content", function(value, element) {
         ...
         jQuery.validator.addMethod("nonstring", function(value, element) {
              cadena= $('#informe_asunto').val().toLowerCase();
              cadena= trim(cadena);
    ...
    function setear_plantilla(pantilla_id) {
        $('#informe_asunto').val($('#plantilla_informe_asunto_'+pantilla_id).html());
        $('#cke_contents_informe_contenido_<?php echo $semilla; ?> iframe').contents().find('body').html($('#plantilla_informe_contenido_'+pantilla_id).html());
    ...
    function guardar_plantilla() {
        ...
        if($.trim($('#input_plantilla_nueva').val())=='' || $.trim($('#input_plantilla_nueva').val())=='nombre de la plantilla'){
            ...
            asunto= trim($.trim($('#informe_asunto').val().toLowerCase()));
            ...
            if(asunto=='' || /eneltexto/.test(asunto)){
                ...
                var contenido = $('#cke_contents_informe_contenido_<?php echo $semilla; ?> iframe').contents().find('body').html();
        ...
        if(error==false){
            $.ajax({
                ...
                type:'POST',
                dataType:'html',
                data:'tipo_formato_id='+$('#formato_tipo_formato_id').val()+'&plantilla[nombre_plantilla]='+$.trim($('#input_plantilla_nueva').val())+'&plantilla[asunto]='+$.trim($('#informe_asunto').val())+'&plantilla[contenido]='+contenido_ex,
