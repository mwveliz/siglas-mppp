...
class formatoInforme {
    public function executeValidar($formulario, &$messages, &$formato) {
        ...
        // campo_uno = Asunto
        if (!$formulario["informe_asunto"]) {
            $messages = array_merge($messages, array("informe_asunto" => "Campo requerido"));
        } else {
            $formato->setCampoUno($formulario["informe_asunto"]);
        }

        // campo_dos = Contenido
        if (!$formulario["informe_contenido"]) {
            $messages = array_merge($messages, array("informe_contenido" => "Campo requerido"));
        } else {
            $formato->setCampoDos($formulario["informe_contenido"]);
        }
    }
    ...
    public function executeTraer($datos) {
        ...
        $formulario["informe_asunto"] = $datos["campo_uno"];
        $formulario["informe_contenido"] = $datos["campo_dos"];
    ...
    foreach ($receptores as $list_receptores) {
        $receptor = '';
            $receptor.= '<font style="font-size: 12px; font-weight: bold">'.ucwords(strtolower($list_receptores->getpn())).' '.
                    ...
                    $list_receptores->getunombre().'</i><br/><b>'.
                    $list_receptores->getctnombre().'</b>';
            ...
            <table width="450" "center">
                ...
                <tr>
                    <td align="justify"><br/><br/>
                        <p><font size="12"><i>$asunto</i></font></p>
...
EOD;
    ...
    // Lee la plantilla
    $plantilla = file_get_contents(sfConfig::get("sf_root_dir")."/apps/correspondencia/modules/formatos/lib/informe/plantillas/informe".$audi."_odt.rtf");
...
//    echo html_entity_decode($valores['informe_contenido']);
//    echo "<br/><br/><br/>";
    $conten = $texto_puro->htmlRtf($valores['informe_contenido']);
//    echo $texto_puro->htmlRtf($valores['informe_contenido']);
...
//        $fecha = date('d-m-Y h:i:s A', strtotime($correspondencia->getFEnvio()));
    ...
    $plantilla = str_replace('%%ASUNTO%%',$texto_puro->htmlRtf($valores['informe_asunto']),$plantilla);
...
');
    ...
    $nombre_def = "informe_".$correspondencia->getNCorrespondenciaEmisor().".odt";
    ...
    $carpeta = $hash_is;
    mkdir(sfConfig::get("sf_root_dir")."/apps/correspondencia/modules/formatos/lib/informe/plantillas/tmp/$carpeta",0777);
    $nuevoarchivo = sfConfig::get("sf_root_dir")."/apps/correspondencia/modules/formatos/lib/informe/plantillas/tmp/$carpeta/$nombre_def";
    if (!copy($nombre_def, $nuevoarchivo)) {
    echo "Ocurrio un error al generar el documento de word Temporal correspondiente";
    }
    $path_a_tu_doc = sfConfig::get("sf_root_dir")."/apps/correspondencia/modules/formatos/lib/informe/plantillas/tmp/$carpeta";
