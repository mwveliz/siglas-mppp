...
class formatoRevisionPuntoInformacion {
    public function executeValidar($formulario, &$messages, &$formato) {
        
        // campo_uno = Observaciones
        // SE PERMITE QUE QUEDE EN VACIO POR ESO SE COMENTA EL IF 
        if (!$formulario["revision_punto_informacion_observaciones"]) {
            $messages = array_merge($messages, array("revision_punto_informacion_observaciones" => "Campo requerido"));
        } else {
            $formato->setCampoDos($formulario["revision_punto_informacion_observaciones"]);
        }
    }
    ...
    public function executePdf($pdf, $formato, $correspondencia_revision, $emisor, $receptores) {

        //Muestra datos del formato
        $valores = $this->executeTraer($formato);

        $doc_padre= Doctrine::getTable('Correspondencia_Correspondencia')->find($correspondencia_revision->getId());
        
        //SE UTILIZAN LOS DATOS DEL PADRE (PUNTO DE CUENTA)
        $correspondencia = Doctrine::getTable('Correspondencia_Correspondencia')->find($doc_padre->getGrupoCorrespondencia());
        $emisor=Doctrine::getTable('Correspondencia_FuncionarioEmisor')->filtrarPorCorrespondencia($doc_padre->getGrupoCorrespondencia());
        $receptores=Doctrine::getTable('Correspondencia_Receptor')->filtrarPorCorrespondencia($doc_padre->getGrupoCorrespondencia());
    
        $valores2 = Doctrine::getTable('Correspondencia_Formato')->findOneByCorrespondenciaId($doc_padre->getGrupoCorrespondencia());
        $valores_padre = $this->executeTraerPadre($valores2);
        
        $correspondencia_punto = $correspondencia;
        $emisor_punto = $emisor;
        $receptores_punto = $receptores;
        
        $correspondencia_revision = Doctrine::getTable('Correspondencia_Correspondencia')->find($correspondencia->getId());
        ...
        if($correspondencia_revision){
            if ($correspondencia_revision->getFEnvio() == null) {
                $f_decision = '<b style="color: red;">SIN FIRMAR</b>';
                $letracolor_revision = '#000000';
                $bgcolor_revision = '#bfbfbf';
                $bgcolor_revision_alterna = '#bfbfbf';
            }else {
...
//        $pdf->setFooterFont(array(PDF_FONT_NAMEs_DATA, '', PDF_FONT_SIZE_DATA));
       ...
       $contenido2= '<table width="525" border="1" cellpadding="4">
                ...
                <tr style="height: 25px">
                    <td colspan="5">'. $valores_padre["punto_informacion_asunto"] .'</td>
                </tr>
                <tr style="background-color: #B40404; color: #FFFFFF;">
                    <td colspan="5">SINTES&Iacute;S</td>
                </tr>
                <tr style="height: 30px">
                    <td colspan="5">'. $valores_padre["punto_informacion_sintesis"] .'</td>
                </tr>
                <tr style="background-color: #B40404; color: #FFFFFF;">
                    <td colspan="5">RECOMENDACIONES</td>
                </tr>
                <tr>
                    <td colspan="5" style="height: 30px">'. $valores_padre["punto_informacion_recomendaciones"] .'</td>
                </tr>
                <tr style="background-color: #B40404; color: #FFFFFF;">
                    <td colspan="5">OBSERVACIONES DE LA MAXIMA AUTORIDAD</td>
                </tr>
                <tr>
                    <td colspan="5" style="height: 70px" bgcolor="'.$bgcolor_revision.'">'. $valores['revision_punto_informacion_observaciones'] .'</td>
                </tr>
                <tr>
