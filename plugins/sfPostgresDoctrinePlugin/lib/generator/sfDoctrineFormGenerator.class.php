...
class sfDoctrineFormGenerator extends sfGenerator
{
  ...
  public function generate($params = array())
  {
    ...
    $pluginPaths = $this->generatorManager->getConfiguration()->getAllPluginPaths();
  	...
  	$files = new RecursiveIteratorIterator(new RecursiveDirectoryIterator(sfConfig::get('sf_lib_dir') . '/'. $this->params['model_dir_name']),
	                                                        RecursiveIteratorIterator::LEAVES_ONLY);
	$modelsPaths = array();
	foreach ($files as $file) {
	  $e = explode('.', $file->getFileName());
	  $modelsPaths[$e[0]] = str_replace(str_replace("\\", "/", sfConfig::get('sf_lib_dir'))."/".$this->params['model_dir_name']."/doctrine", '', str_replace("\\", "/", $file->getPath()));
	}

	$omit = array("sfPostgresDoctrineRecord", "BaseDoctrineRecord");
    // create a form class for every Doctrine class
    foreach ($models as $model)
	{
		if (!in_array($model, $omit)) {
	      ...
	      if ($isPluginModel)
	      {
	      	...
	      	{
	        	mkdir($baseDir.'/plugins', 0777, true);
	      	}
  ...
  public function getValidatorClassForColumn($column)
  {
    ...
    return sprintf('sf'.'Validator%s', $validatorSubclass);
  }
