...
##class## extends sfPostgresDoctrineRecord
{
}
...
EOF;
    ...
    if (!file_exists($baseRecordFile)) {
    	file_put_contents($baseRecordFile, str_replace('##class##', 'class BaseDoctrineRecord', $baseRecordFileContent));
    }

    $baseTableFile = sprintf("%s/BaseDoctrineTable.class.php", $config['models_path']);
    $baseTableFileContent = <<<TABLE
<?php
...
TABLE;
    ...
    foreach (sfYaml::load($schema) as $model => $definition)
    {
      ...
      $matches = array();
      if (preg_match_all('/@property (\w+) \$(\w+)/', $code, $matches, PREG_SET_ORDER) || preg_match('/\.Entities/', $definition['package']))
      {
        ...
        if (!file_exists(str_replace(sprintf('%s/Base', $builderOptions['baseClassesDirectory']), '', $newfile))) {
        	$listFiles[sprintf('%s%s/%s%s', $config['models_path'], isset($definition['package']) ? '/'.substr($definition['package'], 0, strpos($definition['package'], '.')) : '', $model, $builderOptions['suffix'])] =
			str_replace(sprintf('%s/Base', $builderOptions['baseClassesDirectory']), '', $newfile);
        }
        else {
    ...
    foreach (array_diff($tableFinder->in($config['models_path']), $before) as $file)
    {
      ...
      if (strpos($file, 'BaseDoctrineTable.class.php') === true) {
      	file_put_contents($file, str_replace('extends BaseDoctrineTable', 'extends Doctrine_Table', sfToolkit::stripComments($contents)));
      } else {
      	file_put_contents($file, str_replace('extends Doctrine_Table', 'extends BaseDoctrineTable', sfToolkit::stripComments($contents)));
      }      
    }
    
    
    if (!file_exists($baseTableFile)) {
        file_put_contents($baseTableFile, str_replace('##class##', 'class BaseDoctrineTable', $baseTableFileContent));
    }
    else {
    	$contents = file_get_contents($baseTableFile);
    	file_put_contents($baseTableFile, str_replace('extends BaseDoctrineTable', 'extends Doctrine_Table', sfToolkit::stripComments($contents)));
    }
